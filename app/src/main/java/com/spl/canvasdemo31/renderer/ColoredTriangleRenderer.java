 ...
 *
 */
public class ColoredTriangleRenderer implements Renderer{
	...
	public void onSurfaceCreated(GL10 gl, EGLConfig config) {
		...
		// 在硬件内存中准备好三角形的数据
		ByteBuffer byteBuffer = ByteBuffer.allocateDirect(3 * (2+4) * 4);
		byteBuffer.order(ByteOrder.nativeOrder());
		// 填充数据
		vertices = byteBuffer.asFloatBuffer();//// 将字节转化为浮点
		vertices.put(new float[]{
			100, 300, 1, 0, 0 ,1, // 左下角 这里只有x,y OpenGL会自动设置z=0
	               1000, 300, 0, 1, 0 ,1,//右下角
	               550, 1700,0, 0, 1 ,1
				});
	...
	public void onSurfaceChanged(GL10 gl, int width, int height) {
        ...
        //vertices 是浮点缓冲区
		vertices.position(0);
        gl.glVertexPointer(2, GL10.GL_FLOAT, 6*4, vertices);// 把顶点位置传输到GPU并储存
       
        vertices.position(2);            
        gl.glColorPointer(4, GL10.GL_FLOAT, 6*4, vertices);// 告知GL颜色在缓冲区的位置
	...
	public void onDrawFrame(GL10 gl) {
		// 表面层的绘制(每秒调用多次)
		gl.glClearColor(1, 1, 1, 1);// 设置清屏颜色
